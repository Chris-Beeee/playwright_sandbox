...
def test_tmdb_now_playing_verification(page, request, record_property):
    is_mock_mode = False
    ...
    try:
        ...
        api_msg = f"[{mode_str}] Successfully retrieved {len(backend_titles)} 'Now Playing' movies from the backend."
        print(f"\n{api_msg}")
    ...
    try:
        ...
        ui_url = "https://www.themoviedb.org/movie/now-playing"
        now_playing_page = GenericMoviesPage(page, ui_url)
        now_playing_page.load()
        now_playing_page.accept_cookies()
        
        scraped_titles = now_playing_page.get_movie_titles()
        ui_msg = f"[UI] Successfully scraped {len(scraped_titles)} movie titles from the web page."
        print(ui_msg)
